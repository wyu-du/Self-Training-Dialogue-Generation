...
class DataReader(object):
    def __init__(self, seed, domain, obj, 
        ...
        self._loadVocab(vocabfile) # list of tokens
        self._loadTokenMap() # dict of sv, e.g. {"sv.address._1": SLOT_ADDRESS}
        
        ## set input feature cardinality
        self._setCardinality() # self.dfs = [0, # of a, # of sv, # of s, # of v]
    ...
    def delexicalise(self,sent,dact):
        feat = self.formatter.parse(dact,keepValues=True) # {'acttype':a,'s2v':[(s,v), (s,?), (None, None)]}
        return self.lexicaliser.delexicalise(sent,feat['s2v']) # replace v in sent with SLOT_s
    ...
    def _loadData(self,filename,group=True,multiref=False):
        ...
        for dact,sent,base in data:
            ...
            sent = self.delexicalise( 
                    normalize(re.sub(' [\.\?\!]$','',sent)),dact) # replace values with SLOT_NAME
            base = self.delexicalise(
                    normalize(re.sub(' [\.\?\!]$','',base)),dact)
            feat = self.formatter.format(dact) # [('a', 'suggest'), ('weightrange', '_1'), ('weightrange', 'dontcare')]
    ...
    def _loadTokenMap(self,mapfile='utils/resource/detect.pair'):
        ...
        for k,v in self.tokenMap:
            for x in ['1','2','3']:
                key = 'sv.'+k+'._'+x
                self.feat2token[key] = v # "sv.address._1": SLOT_ADDRESS
    ...
    def genFeatVec(self,feat,cardinality,dfs):
        '''
        Input:
            - feat: [('a', 'suggest'), ('weightrange', '_1'), ('weightrange', 'dontcare')], given by current data
            - cardinality: [a.suggest, sv.acceptscreditcards.dontcare], given by `feat_template.txt`
            - dfs: [0, # of a, # of sv, # of s, # of v], given by cardinality
        Return:
            - a: [act_id_1, act_id_1]
            - sv: [sv_id_1, sv_id_2]
            - s: [s_id_1, s_id_2]
    ...
    def check(self, dact, text):
        sent = self.delexicalise( 
                normalize(re.sub(' [\.\?\!]$','',text)), dact) # replace values with SLOT_NAME
        feat = self.formatter.format(dact) # [('a', 'suggest'), ('weightrange', '_1'), ('weightrange', 'dontcare')]
        a,sv,s,v = self.genFeatVec(feat,self.cardinality,self.dfs)
        a,sv,s,v =  np.array(a,dtype=np.int32),\
                    np.array(sv,dtype=np.int32),\
                    np.array(s,dtype=np.int32),\
                    np.array(v,dtype=np.int32)
        return a,sv,s,v,sent