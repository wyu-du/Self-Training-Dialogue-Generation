    ...
    def parse(self,dact,keepValues=False):
        ...
        for dact in dacts:
            if ' = ?' in dact:
                dact = dact.replace(' = ?', '')
                dact = '?'+dact
            if ' none = none ' in dact:
                dact = dact.replace(' none = none ', '')
            acttype = dact.split('(')[0].strip()
            jsact['acttype'].append(acttype)
            ...
            for slt2val in slt2vals:
                if slt2val=='': # no slot
                    ...
                    jsact['s2v'].append( (slt2val.strip('\'\"'),'?') ) # 's2v': (s, ?)
                    ...
                    if  not v in self.special_values.keys() and\
                        ...
                        v = '_'
                    jsact['s2v'].append((s,v)) # 's2v': (s, _)
        jsact['acttype'] = list(set(jsact['acttype']))
        return jsact
